...
const Animations = (() => {
    ...
    const initialize = () => {
        try {
            ...
            setTimeout(() => {
                ...
                // Setup header logo animation sequence
                setupLogoAnimation();
    ...
    const setupStateAnimations = () => {
        try {
            ...
            document.body.addEventListener('click', (e) => {
                ...
                // Note: Tab switching animations are now handled by the UI.setupTabNavigation function
                // This is just for mobile nav animation during tab switches
                const navButton = e.target.closest('.nav-button');
                if (navButton) {
                    // Add tab-switching class to the mobile nav for animation
                    const mobileNav = document.querySelector('.mobile-nav');
                    if (mobileNav) {
                        mobileNav.classList.add('tab-switching');
                        setTimeout(() => {
                            mobileNav.classList.remove('tab-switching');
                        }, 600);
                    }
                }
            });
        ...
        try {
            ...
            setTimeout(() => {
                ...
                const elementsToAnimate = document.querySelectorAll(
                    ...
                    '.pic-grid-item, .goal-item, .exercise-item'
                );
                
                // Immediately make start-option and template-card fully visible with no animations
                document.querySelectorAll('.start-option, .template-card').forEach(element => {
                    element.classList.add('scroll-animate');
                    element.style.opacity = '1';
    ...
    const setupLogoAnimation = () => {
        try {
            ...
            console.log('Logo animation sequence initialized');
        } catch (error) {
            console.warn('Error setting up logo animation:', error);
        }
    };
