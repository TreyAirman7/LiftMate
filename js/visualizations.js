...
const createMuscleMapSvg = () => {
    ...
    const renderVolumeDistribution = () => {
        ...
        // Update colorPalette with current theme colors
        colorPalette = getThemeColors();
        ...
        // Get all muscle groups with volume data
        ...
        // Use all muscle groups, don't limit
        ...
        document.documentElement.style.setProperty('--chart-background', isDarkMode ? '#000000' : '#FFFFFF');
        ...
        const barColors = sortedGroups.map((_, i) => {
            // Get colors from the theme palette
            const baseColors = [
                colorPalette.primary,
                colorPalette.secondary,
                colorPalette.tertiary,
                'rgba(46, 125, 50, 1)',   // Success
                'rgba(13, 101, 116, 1)',  // Teal
                'rgba(68, 138, 255, 1)',  // Blue
                'rgba(213, 0, 249, 1)',   // Purple
                'rgba(197, 17, 98, 1)',   // Pink
                'rgba(239, 108, 0, 1)',   // Orange
                'rgba(192, 202, 51, 1)',  // Lime
                'rgba(0, 77, 64, 1)',     // Dark Teal
                'rgba(183, 28, 28, 1)'    // Dark Red
            ];
            
            // Repeat colors if we have more muscle groups than colors
        ...
        charts.volumeDistribution = new Chart(ctx, {
            ...
            options: {
                responsive: true,
                maintainAspectRatio: false,
                aspectRatio: 0.75, // Make the chart 25% taller (default is 1.0)
                layout: {
                    padding: {
                        top: 30,
                        bottom: 30,
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        backgroundColor: isDarkMode ? 'rgba(255, 255, 255, 0.1)' : null, // Subtle background in dark mode
                        textStrokeWidth: 0, // No stroke
                        font: {
                            weight: 'bold',
                            size: 28  // Double the font size
                        },
                        padding: {
                            top: 5,
                            bottom: 5
                    ...
                    legend: {
                        display: true,
                        position: 'bottom', // Move to bottom to give more space for the chart
                        align: 'center',
                        maxHeight: 400, // Allow more height for legend
                        labels: {
                            boxWidth: 20, // Smaller box to fit more items
                            padding: 15, // Less padding
                            color: isDarkMode ? '#FFFFFF' : '#000000', // Change based on mode
                            textStrokeWidth: 0, // No stroke
                            backgroundColor: isDarkMode ? 'rgba(255, 255, 255, 0.1)' : null, // Subtle background in dark mode
                            borderRadius: 0, // No rounded corners
                            font: {
                                size: 32, // Double the font size
                                weight: 'bold'
                            },
                            usePointStyle: true,
                    ...
                    datalabels: {
                        ...
                        color: '#FFFFFF', // Force white color for percentages
                        font: {
                            weight: 'bold',
                            size: 16, // Dramatically reduced size (25% of 65)
                        ...
                        borderRadius: 0,
                        padding: 4,
                        // Keep text oriented to face the user
                        rotation: 0, // No rotation - always face user
                        // Move labels to the outside of the pie
                        position: 'outside',
                        anchor: 'end', // End of the radius line
                        align: 'end', // End of the radius line
                        offset: 12, // Reduced offset to move inward by 3px
                        clamp: true, // Prevent labels from overlapping the center
                        display: true
                    },
                    tooltip: {
                        callbacks: {
                ...
                layout: {
                    padding: {
                        ...
                        bottom: 30,
                        left: 30,
                        right: 120 // More padding on right for legend
                    }
                },
                radius: '60%' // Smaller pie to leave more space for the legend and labels
            }
        });
    };
    ...
    const renderRepRangesChart = () => {
        ...
        // Define distinct colors for the different rep ranges (not matching theme)
        const repColors = [
        ...
        const totalSetsValue = totalSets || 1; // Prevent division by zero
        const strengthPercentage = Math.round((repRanges['Strength (1-5)'] / totalSetsValue) * 100);
        const hypertrophyPercentage = Math.round((repRanges['Hypertrophy (6-12)'] / totalSetsValue) * 100);
        const endurancePercentage = Math.round((repRanges['Endurance (13+)'] / totalSetsValue) * 100);
        
        const ctx = canvas.getContext('2d');
        charts.repRanges = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['Rep Range Distribution'],
                datasets: [
                    {
                        label: 'Strength (1-5 reps)',
                        data: [repRanges['Strength (1-5)']],
                        backgroundColor: repColors[0],
                    ...
                    {
                        label: 'Hypertrophy (6-12 reps)',
                        data: [repRanges['Hypertrophy (6-12)']],
                        backgroundColor: repColors[1],
                        borderColor: isDarkMode ? 'rgba(0, 0, 0, 0.1)' : 'rgba(255, 255, 255, 0.1)',
                        borderWidth: 1,
                        borderSkipped: false,
                        borderRadius: 0,
                    ...
                    {
                        label: 'Endurance (13+ reps)',
                        data: [repRanges['Endurance (13+)']],
                        backgroundColor: repColors[2],
                        borderColor: isDarkMode ? 'rgba(0, 0, 0, 0.1)' : 'rgba(255, 255, 255, 0.1)',
                        borderWidth: 1,
                        borderSkipped: false,
                        borderRadius: {
            ...
            options: {
                ...
                responsive: true,
                maintainAspectRatio: false,
                aspectRatio: 0.25, // Make the chart much taller (half the original aspect ratio)
                layout: {
                    padding: {
                        top: 60,
                        bottom: 60,
                ...
                plugins: {
                    ...
                    datalabels: {
                        ...
                        formatter: function(value, context) {
                            // Show percentages
                            const percentages = [strengthPercentage, hypertrophyPercentage, endurancePercentage];
                            return percentages[context.datasetIndex] + '%';
                        },
                        color: isDarkMode ? '#FFFFFF' : '#000000',
                        font: {
                            weight: 'bold',
                            size: 100 // Much larger text for percentages
                        },
                        textStrokeWidth: 3, // Subtle stroke for visibility
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                const percentage = Math.round((value / totalSetsValue) * 100);
                ...
                barThickness: 120 // Double the bar thickness
            }
        });
    };
