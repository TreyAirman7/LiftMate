...
Chart.register({
    ...
    beforeDraw: (chart) => {
        ...
        const statsTab = document.querySelector('.tab-content#statistics.active');
        
        // Only apply background to charts in the stats tab
        if (statsTab && statsTab.contains(chart.canvas)) {
            const ctx = chart.canvas.getContext('2d');
            ctx.save();
            ctx.globalCompositeOperation = 'destination-over';
            ctx.fillStyle = 'rgba(240, 240, 240, 1)'; // Light grey background for stats tab charts
            ctx.fillRect(0, 0, chart.width, chart.height);
            ctx.restore();
        } else if (chart.options.plugins.customCanvasBackgroundColor?.color) {
            // For other charts, only apply background if explicitly specified
            const ctx = chart.canvas.getContext('2d');
            ctx.save();
            ctx.globalCompositeOperation = 'destination-over';
...
const VisualizationManager = (() => {
        ...
        getMuscleHeatColors: () => {
            ...
            if (rgbMatch) {
                ...
                return [
                    ...
                    'rgba(157, 133, 210, 1)',
                    'rgba(126, 103, 183, 1)',
                    primaryColor    // Highest intensity
                ];
            }
    ...
    const configureChartDefaults = () => {
        ...
        try {
            ...
            Chart.defaults.color = document.body.classList.contains('dark-mode') ? '#FFFFFF' : '#000000';
...
const createMuscleMapSvg = () => {
    ...
    const renderAllVisualizations = () => {
        // renderProgressTimeline('month'); // Timeline feature removed
        renderMuscleHeatmap();
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        ...
                        position: 'right',
                        align: 'start', // Align to the start of the chart
                        maxHeight: 400, // Allow more height for legend
                        labels: {
                            boxWidth: 20, // Smaller box to fit more items
                            padding: 15, // Less padding
                            color: document.body.classList.contains('dark-mode') ? '#FFFFFF' : '#000000', // Change based on mode
                            textStrokeColor: document.body.classList.contains('dark-mode') ? 'rgba(0, 0, 0, 0.8)' : 'rgba(255, 255, 255, 0.4)', // Mode-specific stroke
                            textStrokeWidth: document.body.classList.contains('dark-mode') ? 5 : 3,
                            backgroundColor: 'rgba(255, 255, 255, 0.2)', // Semi-transparent white background
                            borderRadius: 3, // Rounded corners
                            font: {
                                size: 16, // Smaller font to fit more items
                                weight: 'bold'
                            },
                            usePointStyle: true,
                    ...
                    datalabels: {
                        ...
                        textAlign: 'center',
                        textStrokeColor: 'rgba(0, 0, 0, 1)', // Solid black stroke
                        textStrokeWidth: 4,
                ...
                plugins: {
                    legend: {
                        ...
                        labels: {
                            ...
                            borderRadius: 3, // Rounded corners
                            font: {
                                size: 30, // Much larger legend text
                                weight: 'bold'
                            },
                            usePointStyle: true,
                            generateLabels: function(chart) {
