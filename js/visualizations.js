...
const VisualizationManager = (() => {
    ...
    let charts = {};
    let colorPalette = {
        primary: 'rgba(103, 80, 164, 1)', // --md-primary
        primaryLight: 'rgba(103, 80, 164, 0.7)',
        primaryContainer: 'rgba(234, 221, 255, 1)', // --md-primary-container
        secondary: 'rgba(98, 91, 113, 1)', // --md-secondary
        ...
        surface: 'rgba(255, 251, 254, 1)', // --md-surface
        outline: 'rgba(121, 116, 126, 1)', // --md-outline
        
        // Muscle group heat map colors
        muscleHeat: [
    ...
    const configureChartDefaults = () => {
        ...
        // Add responsive animation duration
        Chart.defaults.animation.duration = 600;
        Chart.defaults.animation.easing = 'easeOutQuart';
        
        // Custom tooltip styling
        Chart.defaults.plugins.tooltip.backgroundColor = 'rgba(103, 80, 164, 0.9)';
    ...
    const createVisualizationContainers = () => {
        // Create Progress Timeline container
        const statsContainer = document.querySelector('#statistics .container');
        if (statsContainer) {
            // Add new visualization sections
            const progressTimelineSection = document.createElement('div');
            progressTimelineSection.innerHTML = `
                <div class="section-header">
                    <div class="title-wrapper">
                        <div class="title-icon"><i class="fas fa-calendar-day"></i></div>
                        <h2>Workout Progress Timeline</h2>
                    </div>
                </div>
                <div class="chart-container">
                    <div class="chart-header">
                        <h3>Workout Frequency & Progress</h3>
                        <div class="chart-timeframe-selector">
                            <button class="chart-timeframe-button active" data-chart-timeframe="month">Month</button>
                            <button class="chart-timeframe-button" data-chart-timeframe="quarter">Quarter</button>
                            <button class="chart-timeframe-button" data-chart-timeframe="year">Year</button>
                        </div>
                    </div>
                    ...
                    <div class="muscle-heatmap-container">
                        ...
                        <div class="muscle-group-legend">
                            ...
                            <div class="legend-scale">
                                <div class="legend-color" style="background-color: rgba(233, 236, 239, 1);"></div>
                                <div class="legend-color" style="background-color: rgba(223, 215, 242, 1);"></div>
                                <div class="legend-color" style="background-color: rgba(190, 174, 226, 1);"></div>
                                <div class="legend-color" style="background-color: rgba(157, 133, 210, 1);"></div>
                                <div class="legend-color" style="background-color: rgba(126, 103, 183, 1);"></div>
                                <div class="legend-color" style="background-color: rgba(103, 80, 164, 1);"></div>
                            </div>
            ...
            workoutVolumeSection.innerHTML = `
                <div class="section-header">
                    <div class="title-wrapper">
                ...
                <div class="charts-row">
                    ...
                    <div class="chart-container half-width">
                        <div class="chart-header">
                            <h3>Set Rep Ranges</h3>
                        </div>
                        <canvas id="rep-ranges-chart"></canvas>
                    </div>
                </div>
            `;
            ...
            if (personalRecordsSection) {
                statsContainer.insertBefore(progressTimelineSection, personalRecordsSection);
                statsContainer.insertBefore(muscleGroupSection, personalRecordsSection);
                statsContainer.insertBefore(workoutVolumeSection, personalRecordsSection);
                // workoutFrequencySection has been removed
            } else {
                statsContainer.appendChild(progressTimelineSection);
                statsContainer.appendChild(muscleGroupSection);
                statsContainer.appendChild(workoutVolumeSection);
                // workoutFrequencySection has been removed
            }
        ...
        // Add PR achievements section to progress tab
        const progressContainer = document.querySelector('#progress .container');
        if (progressContainer) {
            const prAchievementsSection = document.createElement('div');
            prAchievementsSection.innerHTML = `
                <div class="section-header">
                    <div class="title-wrapper">
                        <div class="title-icon"><i class="fas fa-award"></i></div>
                        <h2>Personal Record Achievements</h2>
                    </div>
                </div>
                <div class="chart-container">
                    <div id="pr-achievements-container" class="pr-achievements-container">
                        <!-- PR achievements will be rendered here -->
                    </div>
                </div>
            `;
            
            // Add to container before weight tracker
            const weightTracker = progressContainer.querySelector('.weight-tracker');
            if (weightTracker) {
                progressContainer.insertBefore(prAchievementsSection, weightTracker);
            } else {
                progressContainer.appendChild(prAchievementsSection);
            }
        }
    };
...
const createMuscleMapSvg = () => {
        ...
        frontMuscleMap.innerHTML = `
            <svg viewBox="0 0 206.326 206.326" xmlns="http://www.w3.org/2000/svg">
                ...
                <path id="chest" class="muscle-group" d="M92,40 C92,45 92,50 94,55 C96,60 100,65 103,65 C106,65 110,60 112,55 C114,50 114,45 114,40 Z" 
                    data-muscle="chest" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                
                <!-- Shoulders -->
                <path id="shoulders-left" class="muscle-group" d="M87,35 C82,35 77,40 75,45 C73,50 72,55 77,58 C82,55 87,50 87,45 Z" 
                    data-muscle="shoulders" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="biceps-left" class="muscle-group" d="M80,60 C77,62 75,66 74,74 C73,77 75,81 77,82 C80,81 83,79 84,74 C85,68 83,60 80,60 Z" 
                    data-muscle="biceps" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="forearms-left" class="muscle-group" d="M77,84 C72,87 69,91 67,95 C67,97 67,99 69,101 C71,98 74,95 76,92 C78,87 79,85 77,84 Z" 
                    data-muscle="forearms" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="abs-upper" class="muscle-group" d="M97,65 C100,65 106,65 109,65 C109,70 109,75 103,75 C97,75 97,70 97,65 Z" 
                    data-muscle="abs" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="obliques-left" class="muscle-group" d="M95,65 C95,70 95,80 95,95 C93,97 90,99 88,99 C86,97 87,90 88,80 C89,70 90,67 95,65 Z" 
                    data-muscle="obliques" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="quads-left" class="muscle-group" d="M90,115 C90,120 89,127 89,135 C86,142 86,147 90,147 C94,145 96,140 97,135 C98,127 98,120 96,115 C94,112 92,112 90,115 Z" 
                    data-muscle="quads" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="calves-left" class="muscle-group" d="M93,146 C90,151 89,161 91,181 C93,194 95,204 95,146 Z" 
                    data-muscle="calves" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
        ...
        backMuscleMap.innerHTML = `
            <svg viewBox="0 0 206.326 206.326" xmlns="http://www.w3.org/2000/svg">
                ...
                <path id="traps-left" class="muscle-group" d="M84,32 C89,30 97,28 100,28 C101,29 101,30 101,32 C101,34 100,36 97,38 C94,39 89,40 85,40 C83,39 82,36 82,35 C82,34 83,33 84,32 Z" 
                    data-muscle="traps" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="upper-back" class="muscle-group" d="M88,47 C93,49 97,52 103,52 C109,52 113,49 118,47 C117,52 116,57 113,62 C108,65 105,67 103,67 C101,67 96,65 93,62 C90,57 89,52 88,47 Z" 
                    data-muscle="back" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="lats-left" class="muscle-group" d="M90,50 C92,52 93,54 95,56 C94,58 92,60 89,62 C87,63 86,64 88,65 C89,66 90,65 91,63 C93,60 92,58 92,56 C93,54 92,52 90,50 Z" 
                    data-muscle="lats" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="triceps-left" class="muscle-group" d="M79,43 C76,46 75,50 74,54 C73,58 73,62 73,68 C72,72 74,75 76,76 C79,75 81,72 82,68 C83,62 84,58 84,54 C84,50 82,46 79,43 Z" 
                    data-muscle="triceps" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="glutes" class="muscle-group" d="M87,100 C82,106 82,114 87,119 C92,123 102,123 103,119 C104,123 114,123 119,119 C124,114 124,106 119,100 C114,96 104,96 103,100 C102,96 92,96 87,100 Z" 
                    data-muscle="glutes" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
                ...
                <path id="hamstrings-left" class="muscle-group" d="M90,123 C95,125 100,127 102,127 C102,130 102,135 102,141 C100,144 96,146 90,146 C88,144 87,141 87,137 C87,133 88,128 90,123 Z" 
                    data-muscle="hamstrings" fill="${colorPalette.muscleHeat[0]}" data-intensity="0"/>
    ...
    const applyMuscleHeatmap = (muscleUsage) => {
        ...
        document.querySelectorAll('.muscle-group').forEach(element => {
            ...
            if (muscleName && muscleUsage[muscleName]) {
                ...
                element.setAttribute('fill', colorPalette.muscleHeat[intensity]);
                ...
                element.addEventListener('mouseover', (e) => {
                    ...
                    e.target.setAttribute('stroke', colorPalette.primary);
    ...
    const renderVolumeDistribution = () => {
        ...
        for (const [muscle, data] of Object.entries(muscleUsage)) {
            if (data.totalVolume > 0) {
                // Format muscle name for display
                const formattedName = muscle.charAt(0).toUpperCase() + muscle.slice(1);
                muscleGroups.push(formattedName);
                volumeData.push(data.totalVolume);
            }
        }
        
        // Sort by volume (descending)
        const combined = muscleGroups.map((group, index) => ({ group, volume: volumeData[index] }));
        combined.sort((a, b) => b.volume - a.volume);
        
        const sortedGroups = combined.map(item => item.group);
        const sortedVolumes = combined.map(item => item.volume);
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                return `${label}: ${percentage}% (${new Intl.NumberFormat('en-US').format(value)} volume)`;
                            }
                        }
                    }
                },
    ...
    const renderRepRangesChart = () => {
        const canvas = document.getElementById('rep-ranges-chart');
        if (!canvas) return;
            ...
            workout.exercises.forEach(exercise => {
                exercise.sets.forEach(set => {
                    if (set.reps) {
                        ...
                        if (set.reps <= 5) {
                            repRanges['Strength (1-5)']++;
                        } else if (set.reps <= 12) {
                            repRanges['Hypertrophy (6-12)']++;
                        } else {
                            repRanges['Endurance (13+)']++;
                        }
                    }
                });
            });
        ...
        // Create the chart
        if (charts.repRanges) {
            charts.repRanges.destroy();
        }
        
        const ctx = canvas.getContext('2d');
        charts.repRanges = new Chart(ctx, {
            type: 'doughnut',
            data: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                const label = context.label || '';
                                const value = context.parsed || 0;
                                const percentage = Math.round((value / totalSets) * 100);
                                return `${label}: ${percentage}% (${value} sets)`;
                            }
                        }
                    }
                },
    ...
    const renderCalendarHeatmap = () => {
        ...
        document.querySelectorAll('.calendar-day').forEach(cell => {
            ...
            if (dateKey && workoutsByDate[dateKey]) {
                ...
                // Apply color
                cell.style.backgroundColor = colorPalette.muscleHeat[intensityLevel];
